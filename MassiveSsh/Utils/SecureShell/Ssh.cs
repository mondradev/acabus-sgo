...
namespace MassiveSsh.Utils.SecureShell
{
    ...
    public class Ssh : IDisposable
    {
        ...
        public String SendCommand(String command, Boolean clearHistory = true)
        {
            ...
            // Intentamos quitar todos los caracteres en el buffer actualmente (2 intentos)
            ClearBuffer();
            Thread.Sleep(80);
            ClearBuffer();
        ...
        private string ProcessReponse(string result)
        {
            ...
            return new Regex(String.Format("{0}|\n{1}|{2}", _BEGIN_RESPONSE_PATTERN, _END_RESPONSE_PATTERN,
                "\\[[0-9]{1,2}[;0-9]{0,3}m")).Replace(result, "");
        }
        ...
        private int ReadResponse(out String response)
        {
            ...
            while (this.IsConnected())
            {
                ...
                String regex = String.Format("{0}([^'])(.*)([^']){1}", _BEGIN_RESPONSE_PATTERN, _END_RESPONSE_PATTERN);
                String text = responseBuilder.ToString().Replace("\r", "").Replace("\n", "");
                if (Regex.IsMatch(text, regex) || Regex.IsMatch(text, "\\<i\\>\\<f\\>"))
