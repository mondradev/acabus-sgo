...
namespace Acabus.Modules.CctvReports.Services
{
    ...
    {
        public static void CreateIncidence(this IList<Incidence> incidences, string description, Device device, DateTime startTime, Priority priority, Location location, string whoReporting)
        {
        ...
        {
            var response = SQLiteAccess.ExecuteQuery("SELECT * FROM Incidences WHERE status=0 UNION SELECT * FROM Incidences WHERE status=1 ORDER BY folio DESC LIMIT 100");
        ...
        public static void SaveInDataBase(this Incidence incidence)
        {
            var query = String.Format("UPDATE Incidences SET WhoReporting='{0}', FinishDate='{1}', Status={2}, Technician='{3}', Observations='{4}', Priority={5} WHERE Folio='{6}'",
            ...
            if (SQLiteAccess.Execute(query) > 0) return;
