...
namespace ACABUS_Control_de_operacion
{
    public partial class Monitor : Form
    {
        ...
        private void RefreshMonitor()
        {
            ...
            {
                Trace.WriteLine(String.Format("El servidor {0} no se encuentra disponible", host), "ERROR");
            }

            this._multiThread.RunTask("Disconnection Vehicles Thread", () =>
            {
            ...
            this._multiThread.RunTask("Fauiler Counters Thread", () =>
             {
                 String[][] response;
                 PostgreSQL psql = PostgreSQL.CreateConnection(host, 5432, "postgres", "admin", "SITM");
                 psql.SetConnectionBySsh(psql.ConnectionBySsh.Replace("/opt/PostgreSQL/9.3/", "/opt/PostgresPlus/9.3AS/"));
            ...
            this._multiThread.RunTask("Trunk Alarms Thread", () =>
             {
                 String[][] response;
                 PostgreSQL psql = PostgreSQL.CreateConnection(host, 5432, "postgres", "admin", "SITM");
                 psql.SetConnectionBySsh(psql.ConnectionBySsh.Replace("/opt/PostgreSQL/9.3/", "/opt/PostgresPlus/9.3AS/"));
